...
void pre_auton()
{
  ...
  displayLCDCenteredString(1, "G | E | L");
...
void liftUp(int power, bool active) {
	if (active == true) {
		if(abs(SensorValue[in8]) != threshold) {
			if(SensorValue[in8] < threshold) {
				motor[liftRightTop] = power;
				motor[liftRightBack] = power;

				motor[liftLeftBack] = 127+ gyroFraction * SensorValue[in8];
				motor[liftLeftTop] = 127+ gyroFraction * SensorValue[in8];
			}
			else if(SensorValue[in8] > threshold) {
				motor[liftLeftBack] = power;
				motor[liftLeftTop] = power;

				motor[liftRightTop] = 127 - gyroFraction * SensorValue[in8];
				motor[liftRightBack] = 127 - gyroFraction * SensorValue[in8];
			}
		}
		else {
			motor[liftLeftBack] = power;
